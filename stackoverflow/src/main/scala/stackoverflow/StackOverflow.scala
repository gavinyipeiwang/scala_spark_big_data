...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    val questions = postings.filter(_.postingType == 1).map(question => (question.id, question))
    val answers = postings.filter(_.postingType == 2).filter(_.parentId.isDefined).map(answer => (answer.parentId.get, answer))
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    val vectors = scored.map(s => (firstLangInTag(s._1.tags, langs).getOrElse(0) * langSpread, s._2))
    vectors.cache()
    vectors
  }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      val index = vs.groupBy(_._1).maxBy(_._2.size)._1
      val langLabel: String = langs(index / langSpread)
      val clusterSize: Int = vs.size
      val langPercent: Double = 100d * vs.count(_._1 == index) / clusterSize
      val medianScore: Int = medianCalculator(vs.map(_._2).toList)
