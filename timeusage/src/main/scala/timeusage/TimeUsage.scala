...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val group1 = """^(t01|t03|t11|t1801|t1803).*$""".r
    val group2 = """^(t05|t1805).*$""".r
    val group3 = """^(t02|t04|t06|t07|t08|t09|t10|t12|t13|t14|t15|t16|t18).*$""".r
    columnNames.foldLeft((List.empty[Column], List.empty[Column], List.empty[Column]))((acc, x) => x match {
      case group1(v) => (new Column(x) :: acc._1, acc._2, acc._3)
      case group2(v) => (acc._1, new Column(x) :: acc._2, acc._3)
      case group3(v) => (acc._1, acc._2, new Column(x) :: acc._3)
      case _ => acc
    })
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when('telfs >= 1.0 && 'telfs < 3.0, "working").otherwise("not working").as("working")
    val sexProjection: Column = when('tesex === 1.0, "male").otherwise("female").as("sex")
    val ageProjection: Column = when('teage.between(15.0, 22.0), "young").when('teage.between(23.0, 55.0), "active").otherwise("elder").as("age")
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(timeUsageRow => TimeUsageRow(
      timeUsageRow.getAs("working"),
      timeUsageRow.getAs("sex"),
      timeUsageRow.getAs("age"),
      timeUsageRow.getAs("primaryNeeds"),
      timeUsageRow.getAs("work"),
      timeUsageRow.getAs("other"))
    )
